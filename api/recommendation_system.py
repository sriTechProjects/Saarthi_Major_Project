...
def main():
    ...
    # Build user-item interaction matrix R (Section 3.1.2)
    R, buyer_to_index, product_to_index = build_interaction_matrix(buyers, products, interactions)
    # Matrix factorization via SGD (Equations (1), (2), (8), (9))
    ...
    print("Select a buyer for whom to generate recommendations:")
    ...
    print(f"\nTop recommendations for buyer {selected_buyer}:")
    for prod_id, score in recommendations[:5]:
        product_name = next((p['name'] for p in products if p['_id'] == prod_id), prod_id)
        print(f"• {product_name} (ID: {prod_id}), Score: {score:.3f}")
    
    # ---------------- Bundle Recommendation ----------------
    # Let the buyer choose one of their purchased products to see what can be bought together.
    print("\n--- Bundle Recommendations ---")
    buyer_interactions = [inter['product_id'] for inter in interactions if inter['buyer_id'] == selected_buyer]
    if not buyer_interactions:
        print("No previous interactions available for bundle recommendations.")
    else:
        print("Select one of your purchased products to see similar items (bundled recommendation):")
        unique_products = list(set(buyer_interactions))
        ...
        selected_product = unique_products[prod_choice]
        bundle_recs = get_bundle_recommendations(selected_product, products, top_n=3)
        print(f"\nProducts often bought together with product ID {selected_product}:")
