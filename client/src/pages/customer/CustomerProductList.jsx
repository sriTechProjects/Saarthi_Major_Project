...
//     const { priceRange, ratings, city } = filterValues;
  
//     const matchesPrice =
//       product.price >= priceRange[0] && product.price <= priceRange[1];
  
//     const matchesRating =
//       ratings.length === 0 || ratings.some((r) => product.rating >= r);
...
const CustomerProductList = () => {
  ...
  const isFilterActive = () => {
    return (
      filterValues.priceRange[0] > MIN ||
      filterValues.priceRange[1] < MAX ||
      filterValues.ratings.length > 0 ||
      filterValues.discounts.length > 0 ||
      filterValues.city !== ""
    );
  };

  // Get filtered products only when filters are active
  const getFilteredProducts = () => {
    ...
    return productList.filter((product) => {
      const { priceRange, ratings, discounts, city } = filterValues;
    
      const matchesPrice =
        product.unit_price >= priceRange[0] && product.unit_price <= priceRange[1];
    
      const matchesRating =
        ratings.length === 0 || ratings.some((r) => product.ratings >= r);
      
      const matchesDiscount =
        discounts.length === 0 || discounts.some((d) => product.discount >= d);
    
      const matchesCity = !city || product.seller_id.shopAddress.city === city;
    
      return matchesPrice && matchesRating && matchesDiscount && matchesCity;
    });
  };
  ...
  return (
    <div className="py-5 px-[7vw]">
      <div className="flex gap-4">
              ...
              {isFilterActive() && (
                <div className="p-3 border-b">
                  <button 
                    ...
                    className="w-full py-1 px-2 bg-green-600 text-white rounded-md text-sm"
                  >
                    Clear All Filters
                  </button>
                </div>
        ...
        {/* Right Product Cards Container */}
        <div className="flex flex-col gap-y-2 max-h-[100vh]">
          {/* No products message when filtered list is empty */}
